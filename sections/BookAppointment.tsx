  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Optionally, you can add user-facing error messages here
      }
    } catch (error) {
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input type="text" name="name" id="name" placeholder="Your Name" required value={formData.name} onChange={handleChange} className="w-full bg-gray-800 border border-gray-600 rounded-lg p-3 text-white focus:ring-2 focus:ring-[#fbbf24] focus:border-[#fbbf24] outline-none transition" />
              </div>
              <div>
                <label htmlFor="email-appt" className="sr-only">Email</label>
                <input type="email" name="email" id="email-appt" placeholder="Your Email" required value={formData.email} onChange={handleChange} className="w-full bg-gray-800 border border-gray-600 rounded-lg p-3 text-white focus:ring-2 focus:ring-[#fbbf24] focus:border-[#fbbf24] outline-none transition" />
              </div>
              <div>
                <label htmlFor="mobileNumber" className="sr-only">Mobile Number</label>
                <input type="tel" name="mobileNumber" id="mobileNumber" placeholder="Your Mobile Number" required value={formData.mobileNumber} onChange={handleChange} className="w-full bg-gray-800 border border-gray-600 rounded-lg p-3 text-white focus:ring-2 focus:ring-[#fbbf24] focus:border-[#fbbf24] outline-none transition" />
              </div>
